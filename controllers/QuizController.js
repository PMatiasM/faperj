    ...
    static async create(req, res) {
        ...
        try {
            ...
            for(const quizItem of quiz.itemList) {
                const response = await axios.post('http://localhost:31415/quizItems', quizItem);
            ...
            const verifyTitle = await db.query(`SELECT * FROM quiz WHERE title='${quiz.title}'`);
    ...
    static async readAll(req, res) {
        try{
            ...
            for(const quiz of quizes) {
                ...
                for(const quizItem of quiz.itemList) {
                    const item = await axios.get(`http://localhost:31415/quizItems/id/${quizItem}`);
